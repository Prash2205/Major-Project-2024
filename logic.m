classdef logic < handle
    ...
    methods (Access = public)
        ...
        % New method to apply various watermark attacks
        function applyWatermarkAttack(this, attackType)
            switch attackType
                case 'Noise'
                    % Add logic to apply noise attack
                    this.SourceImage = watermarkAttacks.applyNoise(this.SourceImage, 0.5);
                case 'Salt and Pepper'
                    % Add logic to apply salt and pepper attack
                    this.SourceImage = watermarkAttacks.applySaltPepper(this.SourceImage, 0.05);
                case 'Speckle Noise'
                    this.SourceImage = watermarkAttacks.applySpeckle(this.SourceImage,0.001);
                case 'Sharpen'
                    this.SourceImage = watermarkAttacks.applySharpen(this.SourceImage,0.5);
                case 'Motion'
                    this.SourceImage = watermarkAttacks.motionAttack(this.SourceImage);
                case 'Average Filter'
                    this.SourceImage = watermarkAttacks.averageFilter(this.SourceImage);
                otherwise
                    % 'No Attack' or unrecognized attack type
                    % Add default behavior or do nothing
            end
        end
    end
